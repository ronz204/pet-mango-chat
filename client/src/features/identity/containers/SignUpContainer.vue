...
const { mutate: signUp, isPending, isError, error } = useMutation({
  mutationFn: (data: SignUpRequest) => IdentityProvider.signUp(data),
  onSuccess: ({ token }) => {
  ...
  <div class="flex min-h-screen flex-col items-center justify-center bg-gray-100 px-4 py-12">
    <div class="mb-8 flex flex-col items-center gap-4">
      ...
      <div class="text-center">
        ...
        <p class="mt-1 text-sm text-gray-500">Join Mango today</p>
      </div>
    </div>

    <div class="w-full max-w-md">
      <SignUpForm :loading="isPending" @submit="signUp" />
